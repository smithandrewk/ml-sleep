...
trainloader,testloader = get_dataloaders(train_ids,test_ids,batch_size=512)
model = SleepStageClassifier()
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.AdamW(params=model.parameters(),lr=3e-3,weight_decay=1e-2)
...
for epoch in tqdm(range(10000)):
    ...
    if epoch % validation_frequency_epochs == 0:
        loss, f1, report = evaluate(trainloader, model, criterion, config["device"])
        ...
        loss, f1, report = evaluate(testloader, model, criterion, config["device"])
